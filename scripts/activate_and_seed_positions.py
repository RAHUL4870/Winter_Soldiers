...
def activate_legs():
    ...
    cur = con.cursor()
    cur.execute("UPDATE shipments SET status = 'IN_TRANSIT' WHERE id IN (SELECT id FROM shipments LIMIT 500);")
    cur.execute("UPDATE legs SET status = 'IN_PROGRESS', actual_departure = datetime('now', '-30 minutes') WHERE shipment_id IN (SELECT id FROM shipments WHERE status = 'IN_TRANSIT') AND sequence_number IN (1, 2);")
    con.commit()
...
def check_positions():
    ...
    cur.execute("SELECT leg_id, asset_type, latitude, longitude, provenance, reported_at FROM position_reports LIMIT 5;")
